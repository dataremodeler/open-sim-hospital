...
SELECT
    ...
    SUM(CASE WHEN unplanned_readmit_30_flag = 1 THEN 1 ELSE 0 END) AS readmissions_30d,
    ROUND(
        100.0 * SUM(CASE WHEN unplanned_readmit_30_flag = 1 THEN 1 ELSE 0 END)
              / NULLIF(COUNT(*), 0),
        2
    )                                                      AS readmission_rate_pct
FROM readmissions.readmission_summary;   -- adjust schema name if needed

-- Readmission rate by primary diagnosis category
SELECT
    SUBSTRING(index_admission_diagnosis_code, 1, 3)        AS dx_prefix,
    COUNT(*)                                               AS index_admissions,
    SUM(CASE WHEN unplanned_readmit_30_flag = 1 THEN 1 ELSE 0 END) AS readmits,
    ROUND(
        100.0 * SUM(CASE WHEN unplanned_readmit_30_flag = 1 THEN 1 ELSE 0 END)
              / NULLIF(COUNT(*), 0),
        1
